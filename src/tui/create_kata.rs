//! Create Kata screen for adding new katas through the TUI.
//!
...
/// Create Kata screen state.
pub struct CreateKataScreen {
    // User input fields
...
impl CreateKataScreen {
    /// Creates a new CreateKata screen with available katas for dependencies.
    ///
    /// # Arguments
    ///
    /// * `available_katas` - List of existing kata names that can be used as dependencies
    pub fn new(available_katas: Vec<String>) -> Self {
        Self {
    ...
    /// Renders the create kata screen.
    ...
    fn render_dependencies_field(&self, frame: &mut Frame, area: Rect) {
        ...
        if self.available_katas.is_empty() {
            let paragraph = Paragraph::new("No katas available (create some first!)").block(
                Block::default()
                    .borders(Borders::ALL)
                    .title(title)
                    .title_style(title_style)
    ...
    fn render_footer(&self, frame: &mut Frame, area: Rect) {
        let footer_text = if self.current_field == FormField::Dependencies {
            Line::from(vec![
    ...
    fn render_confirmation(&self, frame: &mut Frame) {
        let chunks = Layout::default()
            .direction(Direction::Vertical)
            .constraints([
                Constraint::Length(10), // Confirmation dialog
    ...
    pub fn handle_input(&mut self, code: KeyCode, exercises_dir: &Path) -> CreateKataAction {
        if self.showing_confirmation {
    ...
    fn handle_confirmation_input(
        ...
        match code {
            KeyCode::Char('y') | KeyCode::Char('Y') => {
                ...
                CreateKataAction::Submit {
                    form_data,
                    slug: self.confirmation_slug.clone(),
                }
            }
    ...
    fn handle_dependencies_input(&mut self, code: KeyCode) {
        ...
        match code {
            KeyCode::Char('j') | KeyCode::Down => {
                if self.dependency_selected_index < self.available_katas.len() - 1 {
            ...
            KeyCode::Char(' ') => {
                // Toggle selection
                let kata = self.available_katas[self.dependency_selected_index].clone();
